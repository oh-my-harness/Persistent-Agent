...
only call bounded planning tools for create-task, split-tasks, pause/resume/cancel/delete task,
manual complete/fail/retry/run-now task, run-next-task, task title/description update, recurring interval update, reprioritize-task, queue-reorder, task-type conversion, dependency add/remove, task-note add,
...
workspace status/file/directory inspection, memory create/list/approve/reject, audit listing, list-waiting-for-user, list-waiting-for-schedule, list-tasks, list-tasks-by-status, summarize, and
...
- `github_create_pull_request` (wired for creating a PR from an already-pushed branch; requires `GITHUB_TOKEN`)

Specialized tools such as browser automation, richer GitHub PR review/merge operations, and richer git
