...
- Axum API with REST endpoints and an SSE event stream for task changes, main-agent replies, main-agent actions, scheduler ticks, and heartbeats.
- Vite React Web UI under `apps/web` with a readable execution monitor for current run state, next queued work, blocked work that needs user input, main-agent action events, and SSE events.
