...
- Main-agent conversation commands in Chinese and English for create with requested skills, split goals, list, create/list/update/delete skill definitions, show task artifacts, show memory candidates, show audit actions, explain state, inspect scheduler/execution state, inspect local workspace status, request clarification, reply to blocked tasks, pause, resume, cancel, reprioritize, reorder, requested-skill changes, dependency changes, resource-lock changes, memory list/approve/reject/update/delete review, scheduler scans, and summarize.
